...
<body>
    ...
    if(count($_GET)!=0){
      if(array_key_exists("id",$_GET) and array_key_exists("status",$_GET)){
        if($_GET["id"]!=NULL and $_GET["status"]!=NULL){
            ...
            if(count($row)!=0){
              $status=$_GET["status"];
              update_comment($comment_id,$status);
            }
            else
              header("Location:./comments.php"); /* Redirect browser */
        }
        else
  ...
  function update_comment($comment_id,$status){
    $conn=connect_db();

    $stmt = $conn->prepare("SELECT article_id,status FROM comments WHERE id=?");
    $stmt->bind_param('i',$comment_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
    ...
    if(($status=="deleted" and $row['status']=="approve") or $status=="notapprove"){

      $stmt = $conn->prepare("UPDATE articles SET comments=comments-1 WHERE id=?");
      $stmt->bind_param('i',$row["article_id"]);
      $stmt->execute();
    }
    if($status=="approve"){
      $stmt = $conn->prepare("UPDATE articles SET comments=comments+1 WHERE id=?");
