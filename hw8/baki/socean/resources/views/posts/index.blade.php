...
<div class="d-flex justify-content-center p-4">
    ...
    <img src="/storage/{{$post[0]->user->profile->image}}" alt="" style="background-image: url('/docs/4.0/assets/brand/bootstrap-solid.svg'); width: 50px; height: 50px; border-radius: 100%; margin-right: 5px"> <a class="text-dark" href="profile.html">{{$post[0]->user->username}}</a>  <p class="d-inline-block float-right text-primary pt-2" style="font-size: 12px"> {{$post[0]->created_at->diffForHumans() }}</p>
    </div>
        <div class="card-body bg-white">
                  <div class="carousel-inner" style="min-height: 400px;max-height: 400px">
                    <div class="carousel-item active" style="position: absolute;top: 50%;-ms-transform: translateY(-50%);transform: translateY(-50%)">
                      <img src="{{ url('/' .$post[0]->image)}}" class="d-block w-100" alt="...">
                    </div>
                  </div>
  ...
  <div class="pl-4">
      <div class="input-group-prepend pb-2">
          <form method="POST" action="/">
              @csrf
              <input hidden type="text" name="post_id" value="{{$post[0]->id}}">
              ...
              <button type="submit" href=""><i class="material-icons pl-3 text-primary" style="font-size:32px;display: inline;">send</i></button>
          </form>
      </div>
  </div>
