...
class chat_gui(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.Frame1.grid(row = 0, column = 0, rowspan = 3, columnspan = 
1, \
        sticky = W+E+N+S)
        self.Frame2 = Frame(master)
        self.Frame2.grid(row = 3, column = 0, rowspan = 3, columnspan = 
1, \
        sticky = W+E+N+S)
        self.Frame3 = Frame(master)
        self.Frame3.grid(row = 0, column = 1, rowspan = 5, columnspan = 
3, \
        sticky = W+E+N+S)
        self.Frame4 = Frame(master)
        self.Frame4.grid(row = 5, column = 1, rowspan = 1, columnspan = 
3, \
        sticky = W+E+N+S)
    ...
    def initialize(self):
        ...
        self.userlist_scrollbar = 
Scrollbar(self.Frame1,orient="vertical")
...
self.gui_userlist.config(yscrollcommand=self.userlist_scrollbar.set)
        ...
        self.connect = 
Button(self.Frame2,text="Connect",command=self.connect)
    ...
    def connect(self):
        if not(self.IS_CONNECTED) and self.server.get() and 
self.port.get() \
        and self.user.get():
            connection = 
client.connect_to_server(self,self.server.get(),\
